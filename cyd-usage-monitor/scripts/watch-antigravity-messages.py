...
Waits in ReadDirectoryChangesW; no timers, screen access, or directory polling.
...
def watch(config_path):
    ...
    def status(value):
        status_path.write_text(json.dumps({"time": time.time(), "status": value,
                                          "pid": os.getpid()}), encoding="utf8")
    ...
    def receive(root, handle):
        ...
        while True:
            ...
            if not ok:
                ...
                return
            if count.value == 0:
                status("overflow")  # fail closed; never replay historical messages
                continue
    ...
    try:
        for raw in config["antigravity_roots"]:
            ...
            threading.Thread(target=receive, args=(root, handle), daemon=True).start()
        if not handles:
            raise ValueError("No Antigravity brain directories")
        status("ready")
        while True:
            path = changes.get()  # sleeps until Windows reports a relevant change
            try:
                config = json.loads(config_path.read_text(encoding="utf8"))
                ...
                result = helper.run("antigravity", event, config,
                                    config_path.parent / "chat-switch.sqlite")
...
def install():
    ...
    command = subprocess.list2cmdline([str(pythonw), str(destination)])
    with winreg.CreateKey(winreg.HKEY_CURRENT_USER,
                         r"Software\Microsoft\Windows\CurrentVersion\Run") as key:
        winreg.SetValueEx(key, "CYDAntigravityMessages", 0, winreg.REG_SZ, command)
    ...
    subprocess.Popen([str(pythonw), str(destination)], cwd=target,
